			...
			<div data-role="header" data-theme="a" data-position="fixed">
			 ...
			 <div data-role="content" data-theme="a">
				...
				if (!empty($_GET['id'])) {
					...
					JOIN mstr_dealer c ON b.id_Dealer=c.id_Dealer WHERE a.id_transaksi='".$_GET['id']."' AND
					a.id_juri= '".$id_juri."' LIMIT 1");
					...
					$dataMax = mysql_fetch_array(mysql_query("SELECT right(`id_transaksi`,4) AS ID  FROM  input_addquestion WHERE id_juri= '".$id_juri."'")); // ambil data maximal dari id transaksi

					if ($dataMax['ID']=='') { // bila data kosong
						$ID = "0001";
						 $resultfaktur = $id_juri.$faktur.$id['hari'].$id['bulan'].$id['tahun'].$ID;
					} else {
						$MaksID = $dataMax['ID'];
						$MaksID++;
						if ($MaksID < 10) {
			...
			?>
					...
					<div data-role="fieldcontain">
						...
						<select name="fbengkel" id="fbengkel" onchange="getTema(this)">
							...
							$sql1 = "SELECT a . * , b.nama_Dealer
									...
									WHERE a.id_Dealer = b.id_Dealer AND NOT EXISTS (SELECT * From input_addquestion as c WHERE c.id_nominee = a.id_nominee AND c.id_juri ='".$row["id_juri"]."')";
					...
					<div data-role="fieldcontain">
					 ...
					 <div data-role="content" data-theme="a">
						...
						<table border = "0" width="100%">
							<tr>
								<td align="">Ide Dealer yang akan di implementasikan :</td>
								<td align="center">
									<select name="fbengkel" id="fbengkel" ">
										<option>[--Pilih Dealer--]</option>
										<?php 
										$sql1 = "SELECT a . * , b.nama_Dealer
												FROM mstr_dealer AS b, mstr_nominee AS a
					...
					<div data-role="footer" data-id="foo1" data-position="fixed">
						<fieldset class="ui-grid-a">
							<div class="ui-block-a"><button type="submit" data-theme="b"  onclick="location.reload(true)">Save</button></div>
							<div class="ui-block-b"><a href="#" onclick="history.go(-1)" data-role="button" data-theme="b">Back</a></div>
						</fieldset>
					</div>
				</form>
			 </div>
